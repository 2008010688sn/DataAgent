...
public interface ModelConfigMapper {
	...
	@Select("""
			SELECT id, provider, base_url, api_key, model_name, temperature, is_active, max_tokens,
			       context_window_tokens, model_type, completions_path, embeddings_path, created_time, updated_time, is_deleted,
			       proxy_enabled, proxy_host, proxy_port, proxy_username, proxy_password
			FROM model_config WHERE is_deleted = false ORDER BY created_time DESC
			""")
	...
	@Select("""
			SELECT id, provider, base_url, api_key, model_name, temperature, is_active, max_tokens,
			       context_window_tokens, model_type, completions_path, embeddings_path, created_time, updated_time, is_deleted,
			       proxy_enabled, proxy_host, proxy_port, proxy_username, proxy_password
			FROM model_config WHERE id = #{id} AND is_deleted = false
			""")
	...
	@Select("""
			SELECT id, provider, base_url, api_key, model_name, temperature, is_active, max_tokens,
			       context_window_tokens, model_type, completions_path, embeddings_path, created_time, updated_time, is_deleted,
			       proxy_enabled, proxy_host, proxy_port, proxy_username, proxy_password
			FROM model_config WHERE model_type = #{modelType} AND is_active = true AND is_deleted = false LIMIT 1
			""")
	ModelConfig selectActiveByType(@Param("modelType") String modelType);

	@Update("UPDATE model_config SET is_active = false WHERE model_type = #{modelType} AND id != #{currentId} AND is_deleted = false")
	...
	@Insert("""
			...
			VALUES (#{provider}, #{baseUrl}, #{apiKey}, #{modelName}, #{temperature}, #{isActive}, #{maxTokens},
			        #{contextWindowTokens}, #{modelType}, #{completionsPath}, #{embeddingsPath}, NOW(), NOW(), false,
	...
	@Update("""
			<script>
			          ...
			          <trim prefix="SET" suffixOverrides=",">
			            ...
			            <if test='contextWindowTokens != null'>context_window_tokens = #{contextWindowTokens},</if>
	...
	@Update("""
			UPDATE model_config SET is_deleted = true WHERE id = #{id}
			""")
