...
-- 为所有表创建触发器
DROP TRIGGER IF EXISTS update_population_total_updated_at ON population_total;
CREATE TRIGGER update_population_total_updated_at
    BEFORE UPDATE ON population_total
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_gender_ratio_updated_at ON gender_ratio;
CREATE TRIGGER update_gender_ratio_updated_at
    BEFORE UPDATE ON gender_ratio
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_age_structure_updated_at ON age_structure;
CREATE TRIGGER update_age_structure_updated_at
    BEFORE UPDATE ON age_structure
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_urban_rural_distribution_updated_at ON urban_rural_distribution;
CREATE TRIGGER update_urban_rural_distribution_updated_at
    BEFORE UPDATE ON urban_rural_distribution
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_province_population_updated_at ON province_population;
...
ON CONFLICT (year, age_group) DO NOTHING;
...
ON CONFLICT (year, province_code) DO NOTHING;

-- 创建视图：综合人口统计视图
CREATE OR REPLACE VIEW population_overview AS
SELECT
    pt.year,
    pt.total_population,
...
-- 创建视图：人口结构分析视图
CREATE OR REPLACE VIEW population_structure_analysis AS
SELECT
        year,
